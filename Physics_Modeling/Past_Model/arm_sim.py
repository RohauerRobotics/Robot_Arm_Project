...
class Plot(object):
	...
	def torque_path(self, t):
		...
		if self.print_torques:
			self.torque1.append(round(np.sqrt((self.orgin_t[0]**2)+(self.orgin_t[1]**2)+(self.orgin_t[2]**2)),3))
			self.torque2.append(round(np.sqrt((self.motor2_t[0]**2)+(self.motor2_t[1]**2)+(self.motor2_t[2]**2)),3))
			self.torque3.append(round(np.sqrt((self.motor3_t[0]**2)+(self.motor3_t[1]**2)+(self.motor3_t[2]**2)),3))
	...
	def loop(self, path):
		...
		for self.x in range(0, self.p):
			...
			if (self.x == (self.p-1)):
				...
				#
				# plt.plot(self.times, self.torque1, color='r')
				# plt.plot(self.times, self.torque2, color='b')
				# plt.plot(self.times, self.torque3, color='g')
				...
				# torques = [self.torque1, self.torque2, self.torque3]
